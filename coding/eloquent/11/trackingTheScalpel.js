...
/*
  ...
  earlier to access storage in arbitrary nests. The scalpel has been
  going around long enough that you may assume that every nest has a
  "scalpel" entry in its data storage.

  Next, write the same function again without using async and await.

  Do request failures properly show up as rejections of the returned
  promise in both versions?
  
  How?

  first version, i didn't even account for possible rejections -- i
  could/should have put a try catch around the anystorage call

  second version, i just have to catch it at any point -> much easier,
  less lines of code
...
async function locateScalpel(nest) {
  // Your code here.
  const content = "scalpel"
  let current = nest.name
  let otherNest = await anyStorage(nest, nest.name, content)
  while (otherNest !== current) {
    console.log(`otherNest: ${otherNest}`)
    current = otherNest
    otherNest = await anyStorage(nest, otherNest, content)
  }
  return otherNest;
}
...
function locateScalpel2(nest) {
  // Your code here.
  return locateScalpelRecurse(nest, nest.name)
}

function locateScalpelRecurse(nest, name) {
  return new Promise((res, rej) => {
    anyStorage(nest, name, 'scalpel').then((otherNestName) => {
      if (name === otherNestName) {
        res(otherNestName)
      } else {
        res(locateScalpelRecurse(nest, otherNestName))
      }
    })
  })
}
