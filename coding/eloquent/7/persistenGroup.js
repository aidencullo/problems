...
-> because we will just create other instances from all the operations
*/
...
class PGroup {
  ...
  add(item){
    return new PGroup(this.array.concat(item));
  }

  delete(item){
    const index = this.array.findIndex((arrayItem) => arrayItem ===
                                       item)
    if (index !== -1) {
  ...
  has(item){
    return  this.array.find((arrayItem) => arrayItem === item) !== undefined;
  }
}
